...
<template>
  <Teleport to="body">
    ...
    >
      <div
        class="bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div class="px-6 pt-6 pb-2 text-center">
          ...
          <h2 id="public-landing-title" class="text-xl font-bold text-slate-100">
            Welcome to Omnijoy
          </h2>
        ...
        <div v-if="view === 'choose'" class="px-6 py-4 space-y-3">
          ...
          <RouterLink
            to="/register"
            class="block w-full text-center rounded-xl bg-slate-800 border border-slate-600 hover:border-indigo-400 hover:text-indigo-300 text-slate-200 font-semibold text-sm px-4 py-2.5 transition-colors"
          >
          ...
          <button
            type="button"
            class="block w-full rounded-xl border border-slate-700 hover:border-indigo-300 hover:bg-indigo-900/50 text-left px-4 py-3 transition-colors"
            @click="view = 'location'"
          >
            <div class="flex items-start gap-3">
              <div class="h-8 w-8 rounded-lg bg-indigo-900/50 text-indigo-400 flex items-center justify-center shrink-0">
        ...
        <div v-else class="px-6 py-4 space-y-4">
          <button
            type="button"
            class="text-xs text-gray-500 hover:text-slate-300 flex items-center gap-1"
          ...
          <div
            v-if="errorMessage"
            class="text-xs text-red-400 bg-red-950 border border-red-100 rounded-lg px-3 py-2"
          >
            {{ errorMessage }}
          </div>
          ...
          <div class="flex flex-col gap-2 pt-1">
            <button
              type="button"
              class="w-full rounded-xl bg-slate-800 border border-indigo-300 hover:bg-indigo-900/50 text-indigo-300 font-semibold text-sm px-4 py-2.5 transition-colors"
            ...
            <button
              type="button"
              class="w-full text-xs text-gray-500 hover:text-slate-300 py-1"
