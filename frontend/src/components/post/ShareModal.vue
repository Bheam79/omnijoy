...
<template>
  <Teleport to="body">
    <div
      ...
      @click.self="close"
    >
      <div class="bg-slate-800 rounded-2xl shadow-xl w-full max-w-md" role="dialog" aria-modal="true" aria-label="Share post">
        <!-- Header -->
        <div class="flex items-center justify-between px-5 py-4 border-b border-slate-700">
          <h2 class="text-lg font-semibold text-slate-100">Share post</h2>
          <button
        ...
        <div class="px-5 py-4 space-y-4">
          <!-- Target selector -->
          <div>
            <label class="block text-sm font-medium text-slate-300 mb-1">Share to</label>
            <select
              v-model="targetType"
          ...
          <div v-if="targetType === 'FriendWall'">
            <label class="block text-sm font-medium text-slate-300 mb-1">Select friend</label>
            <select
              v-model="targetId"
              class="w-full border border-slate-600 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            ...
            <p v-if="friendsStore.loadingFriends" class="text-xs text-slate-500 mt-1">Loading friends…</p>
            <p v-else-if="friendsStore.friends.length === 0" class="text-xs text-slate-500 mt-1">
          ...
          <div v-if="targetType === 'CompanyPage'">
            <label class="block text-sm font-medium text-slate-300 mb-1">Company Page ID</label>
            <input
              v-model="targetId"
              type="text"
