  ...
  <div class="space-y-4">
    ...
    <template v-if="activeTab === 'friends'">
      <div v-if="store.loadingFriends && store.friends.length === 0" class="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div v-for="i in 4" :key="i" class="bg-white rounded-xl p-4 shadow-sm border border-gray-100 animate-pulse flex items-center gap-3">
          <div class="w-12 h-12 rounded-full bg-gray-200 shrink-0" />
          <div class="flex-1 space-y-2"><div class="h-4 bg-gray-200 rounded w-32" /><div class="h-3 bg-gray-100 rounded w-20" /></div>
        </div>
      </div>

      <div v-else-if="store.friends.length === 0" class="text-center py-16">
        <p class="text-4xl mb-3">👥</p>
        <p class="font-semibold text-gray-700">No friends yet</p>
        <p class="text-sm text-gray-500 mt-1">Check out suggestions to find people you may know.</p>
        <button class="mt-4 text-sm font-medium text-indigo-600 hover:underline" @click="activeTab = 'suggestions'">
          Browse suggestions →
        </button>
      </div>
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            <RouterLink :to="`/profile/${friend.user.id}`" class="font-semibold text-gray-900 hover:underline text-sm truncate block">
              {{ friend.user.displayName }}
            </RouterLink>
            <p class="text-xs text-gray-500">
              {{ friend.mutualFriendCount > 0 ? `${friend.mutualFriendCount} mutual` : 'Friends' }}
              <template v-if="friend.familyRelation">
                · <span class="text-indigo-600">{{ friend.familyRelation.subtype }}</span>
              </template>
            </p>
          </div>
        ...
        <button
          ...
          class="col-span-full text-sm text-indigo-600 hover:underline py-3 text-center"
    ...
    <template v-else-if="activeTab === 'requests'">
      <div v-if="store.loadingRequests" class="space-y-3">
        <div v-for="i in 3" :key="i" class="bg-white rounded-xl p-4 shadow-sm border border-gray-100 animate-pulse flex items-center gap-3">
          <div class="w-12 h-12 rounded-full bg-gray-200 shrink-0" />
          <div class="flex-1 space-y-2"><div class="h-4 bg-gray-200 rounded w-32" /><div class="h-3 bg-gray-100 rounded w-24" /></div>
        </div>
      </div>
      ...
      <div v-else class="space-y-3">
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            <RouterLink :to="`/profile/${req.from.id}`" class="font-semibold text-gray-900 hover:underline text-sm">
          ...
          <div class="flex items-center gap-2 shrink-0">
            ...
            <button class="text-sm font-medium bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-1.5 rounded-lg transition" @click="store.declineRequest(req.from.id)">Decline</button>
          </div>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="activeTab === 'sent'">
      ...
      <div v-else class="space-y-3">
        ...
        >
          ...
          <button class="text-sm font-medium bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-1.5 rounded-lg transition" @click="store.cancelRequest(req.from.id)">Cancel</button>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="activeTab === 'suggestions'">
      <div v-if="store.loadingSuggestions" class="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div v-for="i in 4" :key="i" class="bg-white rounded-xl p-4 shadow-sm border border-gray-100 animate-pulse">
          <div class="flex items-center gap-3 mb-3"><div class="w-12 h-12 rounded-full bg-gray-200" /><div class="h-4 bg-gray-200 rounded w-28" /></div>
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        >
          <div class="flex items-center gap-3 mb-3">
            ...
            <div class="flex-1 min-w-0">
              <RouterLink :to="`/profile/${s.user.id}`" class="font-semibold text-gray-900 hover:underline text-sm block truncate">
                {{ s.user.displayName }}
              </RouterLink>
