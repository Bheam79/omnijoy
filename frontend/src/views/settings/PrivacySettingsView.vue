...
<template>
  <div>
    ...
    <div class="flex items-center gap-2 mb-6">
      <RouterLink to="/settings" class="flex items-center gap-1.5 text-sm text-gray-500 hover:text-slate-300 transition-colors">
    ...
    <h1 class="text-2xl font-bold text-slate-100 mb-2">Privacy</h1>
    ...
    <template v-else-if="privacy">
      ...
      <form class="space-y-4" @submit.prevent="save">
        <div
          ...
          :key="field.key"
          class="bg-slate-800 rounded-xl border border-slate-700 px-5 py-4"
        >
          <label class="block text-sm font-medium text-slate-100 mb-0.5">{{ field.label }}</label>
