...
<template>
  <div class="space-y-4">
    ...
    <div class="flex items-center gap-2">
      <label class="text-sm font-medium text-slate-400">Action</label>
      <select
    ...
    <div v-else class="overflow-x-auto bg-slate-800 border border-slate-700 rounded-xl">
      <table class="min-w-full text-sm" aria-label="Audit log">
        ...
        <tbody class="divide-y divide-slate-700">
          <tr v-for="entry in store.auditLog" :key="entry.id" class="hover:bg-slate-700">
            <td class="px-3 py-2 text-gray-500 whitespace-nowrap">
              {{ formatDate(entry.createdAt) }}
            </td>
            <td class="px-3 py-2 text-slate-200">{{ entry.actorDisplayName }}</td>
            <td class="px-3 py-2">
              <span class="inline-block px-2 py-0.5 text-xs font-semibold rounded bg-indigo-900 text-indigo-800">
                {{ entry.action }}
              </span>
            </td>
            ...
            <td class="px-3 py-2 text-slate-300 max-w-xs truncate">
              {{ entry.notes || '—' }}
            </td>
          </tr>
        </tbody>
    ...
    <div v-if="store.auditLog.length > 0" class="flex items-center justify-between">
      ...
      <span class="text-sm text-slate-400">Page {{ store.auditPage }}</span>
      <button
        class="px-3 py-1.5 text-sm font-medium text-slate-300 border border-slate-600 rounded-lg disabled:opacity-50"
